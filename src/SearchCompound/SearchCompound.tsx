...
export default function SearchCompound() {
    ...
    useEffect(() => {
        ...
        //     uses: 'To make water',
        //     chemicalFormula: 'H2',
        //     facts: ['It is the smallest element'],
        //     boilingPoint: '20.271',
        //     meltingPoint: '13.99'
        //     });
        loadCompound();
    ...
    return(
        <div className="p-3 flex items-start justify-start text-white bg-black flex-col min-h-screen w-full max-sm:p-5">
            <Link to="/" className="text-lg text-blue-700 font-bold">Back</Link>
            <div className="flex flex-col gap-5 max-sm:gap-3 border-0 border-white max-sm:border-2 rounded-md p-4">
            ...
            <div className="flex flex-row flex-wrap gap-3">
                <div className="flex flex-row p-1 gap-1">
                    ...
                    <p className="text-lg">{compounds?.boilingPoint}</p> 
                </div>
                <div className="flex flex-row p-1 gap-1">
                    <h1 className="text-xl font-bold">Melting Point: </h1>
                    <p className="text-lg">{compounds?.meltingPoint}</p> 
                </div>
            </div>
            {/* Uses */}
            <div className="flex flex-col gap-3">
                ...
                {compounds?.uses.map((use, index) => (
                    <p className="text-lg my-1" key={index}>{use}</p>
                ))}
            </div>
            {/* Facts */}
            <div className="flex flex-col gap-3">
                ...
                {compounds?.facts.map((fact, index) => (
                    <p className="text-lg" key={index}>{fact}</p>
                ))}
            </div>
            </div>
